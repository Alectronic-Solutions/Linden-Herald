...
export default function IssueContents({
  ...
  if (variant === "compact") {
    return (
      <ul className="space-y-2 font-body text-[0.95rem] text-ink-muted">
        {contents.map((item) => (
    ...
    <ol className="divide-y divide-rule border-y border-rule">
      {contents.map((item) => (
        <li key={item.title} className="flex gap-4 py-4">
          <span
            aria-hidden
            className="mt-1 w-10 shrink-0 font-label text-[0.7rem] uppercase tracking-[0.12em] text-ink-faint"
          >
          ...
          <div className="min-w-0">
            ...
            {item.deck && (
              <p className="mt-1.5 font-body text-[0.95rem] leading-relaxed text-ink-muted">
                {item.deck}
              </p>
            )}
          </div>
